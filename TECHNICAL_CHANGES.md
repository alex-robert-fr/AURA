...
- Grille logique pure dans `grid.ts` séparée du rendu `grid-overlay.ts` ; `fastCheck` activé sur `scene.pick`, `depthFunction = Constants.ALWAYS` sur les materials (évite le second rendering group qui casse le `DefaultRenderingPipeline`), surface d'API réduite au strict nécessaire ([#5](https://github.com/alex-robert-fr/AURA/pull/5))
